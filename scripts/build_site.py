...
def main():
    ...
    save_json(ddata/'history-index.json',idx[:100]); current=store.current_observations(); current.sort(key=lambda x:(x.get('fetched_at',''),x.get('period','')),reverse=True); save_json(ddata/'signals-latest.json',current[:500]); runs=last_runs(store,20); reviews=store.reviews(); reg=load_json(ROOT/'config/sources.json',{}) or {}; overrides=load_json(ROOT/'config/automation_overrides.json',{}) or {}; manual_ids=set(overrides.get('manual_review_only',[])); state=store.load_state(); enabled=[s for s in reg.get('sources',[]) if s.get('enabled',True)]; automated=[s for s in enabled if s.get('id') not in manual_ids]; manual=[s for s in enabled if s.get('id') in manual_ids]; last=runs[0] if runs else None
